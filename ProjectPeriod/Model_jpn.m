...
RADIUS_JNODE = simunits(par.intn.seg.geo.diam.units)*par.intn.seg.geo.diam.value.ref.*ones(nintn,4)/2; %juxtaparanode
...
LENGTH_JNODE = simunits(par.intn.seg.geo.length.units)*par.intn.seg.geo.length.value.ref*ones(nintn,4); % juxtaparanode
...
SURFACEAREA_JNODE = 2*pi*RADIUS_JNODE.*LENGTH_JNODE; % juxtaparanode
...
actcond=cell(1,length(par.node.elec.act)+1);
...
% and for each juxtaparanode segment
actcond{i+1}=simunits(par.intn.elec.act.cond.units)*par.intn.elec.act.cond.value.vec.*SURFACEAREA_JNODE;
actcond{i+1} = reshape(actcond{i+1}', [], 1);
...
gates=cell(1,length(par.node.elec.act)+1);
for i=1:length(par.node.elec.act)
...
% same for the internode (juxtaparanode) channels
gates{end}=cell(1,par.intn.elec.act.gates.number);
    for j=1:par.intn.elec.act.gates.number
        a = rateequation(vrest, par.sim.temp, par.intn.elec.act.gates.temp, par.intn.elec.act.gates.alpha.q10(j), par.intn.elec.act.gates.alpha.equ{j});
        b = rateequation(vrest, par.sim.temp, par.intn.elec.act.gates.temp, par.intn.elec.act.gates.beta.q10(j), par.intn.elec.act.gates.beta.equ{j});
        gates{end}{j}(1:4*nintn, 1) = a/(a+b);
    end
...
gamma=cell(1,length(par.node.elec.act)+1);
delta=cell(1,length(par.node.elec.act)+1);
for i=1:length(par.node.elec.act)
...
% same for the internode (juxtaparanode) channels
gamma{end}=cell(1,par.intn.elec.act.gates.number);
delta{end}=cell(1,par.intn.elec.act.gates.number);
for j=1:par.intn.elec.act.gates.number
    a = rateequation(V, par.sim.temp, par.intn.elec.act.gates.temp, par.intn.elec.act.gates.alpha.q10(j), par.intn.elec.act.gates.alpha.equ{j});
    b = rateequation(V, par.sim.temp, par.intn.elec.act.gates.temp, par.intn.elec.act.gates.beta.q10(j), par.intn.elec.act.gates.beta.equ{j});
    gamma{end}{j}=a./(1/dt+(1/2)*(a+b));
    delta{end}{j}=(1/dt-(1/2)*(a+b))./(1/dt+(1/2)*(a+b));
end
...
erevval = nan(length(par.node.elec.act)+1, 1);
...
% same for the internode (juxtaparanode) channels
for k=1:par.intn.elec.act.gates.number
    numgates(end, k)=par.intn.elec.act.gates.numbereach(k); 
end
erevval(end)=par.intn.elec.act.erev.value;
...
% time-step.
L                                           = [sub2ind(size(A), nodes, nodes), sub2ind(size(A), jpns, jpns)];
offset                                      = A(L);
...
% juxtaparanode
activesum_jpn                                   = zeros(4*nintn, 1);
activesum2_jpn                                  = zeros(4*nintn, 1);
tempprod_jpn                                    = ones(4*nintn, 1);

V1                                          = vrest * repmat([1, 0], tns, 1);
V2                                          = [zeros(tns+2, 1), [[0, 0]; reshape(V1, tns, 2); [0, 0]], zeros(tns+2, 1)];
Vsave                                       = nan(T+1, nns*nnodes+4*nintn);
Vsave(1, :)                                 = V1([nodes,jpns]);
...
for i = 1 : T
    ...
    tableIdxJpn = round((V2(jpns+1, 2) - Vlowerlimit) * (1 / Vstep)) + 1;

    if any(tableIdx > gridsize)
        fprintf('\nToo much stimulation: reduce stimulation\n');
        break
    end
    if any(tableIdx < 1 | isnan(tableIdx))
        fprintf('\nVoltage has gone outside reasonable bounds - possibly incorrect parameter settings\n')
        break
    end
    
    % for juxtaparanode
    if any(tableIdxJpn > gridsize)
        fprintf('\nToo much stimulation: reduce stimulation (jpn)\n');
        break
    end
    if any(tableIdxJpn < 1 | isnan(tableIdxJpn))
        fprintf('\nVoltage has gone outside reasonable bounds - possibly incorrect parameter settings (jpn)\n')
        break
    end
    ...
    % updating juxtaparanode gating variables 
    for k = 1 : par.intn.elec.act.gates.number
        gates{end}{k} = gamma{end}{k}(tableIdxJpn) + delta{end}{k}(tableIdxJpn) .* gates{end}{k};
    end
    ...
    % adding components for the juxtaparanode
    activesum_jpn(:) = 0;
    activesum2_jpn(:) = 0;
    tempprod_jpn(:) = 1;
    for k = 1 : par.intn.elec.act.gates.number
        tempprod_jpn = tempprod_jpn .* (gates{end}{k} .^ numgates(end,k));
    end
    activesum_jpn = activesum_jpn + actcond{end} .* tempprod_jpn / 2;
    activesum2_jpn = activesum2_jpn + actcond{end} .* tempprod_jpn * erevval(end);
    
    % Update active entries of the A matrix.
    A(L) = offset + [activesum; activesum_jpn]';
    ...
    V1(jpns,1)=V1(jpns,1)-activesum_jpn.*V2(jpns+1,2)+activesum2_jpn;
    V1([nodes,jpns],2)=0;
